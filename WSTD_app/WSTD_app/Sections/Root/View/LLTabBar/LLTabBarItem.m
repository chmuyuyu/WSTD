...
- (void)layoutSubviews {
	...
	if (imageSize.width != 0 && imageSize.height != 0) {
        CGFloat imageViewCenterY =0.0;
        if (_tabBarItemType == LLTabBarItemRise) {
            imageViewCenterY = CGRectGetHeight(self.frame) - 3 - titleSize.height - imageSize.height / 2 - 5;
        }else
        {
            imageViewCenterY =(CGRectGetHeight(self.frame) - titleSize.height-3) / 2;
        }
    ...
    self.badgeOriginX = self.imageView.center.x;
    self.badgeOriginY = -self.imageView.center.y/2+5;
